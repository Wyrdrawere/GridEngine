import Input.{CursorPosition, KeyHeld, KeyPressed, KeyReleased, MouseClicked, MouseHeld, MouseReleased}
...
object InputData {
  def update: InputData = {
    updateKeys()
    updateMouse()
    input
  }

  private var input = InputData(Vector2(0), Set.empty, Set.empty, Set.empty, Set.empty, Set.empty, Set.empty)
  ...
  def toList(): List[Input] = {
    ...
    mClick ++ mHold ++ mRelease ++ kPress ++ kHold ++ kRelease :+ pos
  }
}