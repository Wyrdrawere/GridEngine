 ...
 relativePosition: Vector2
  ...
  def drawOnGrid(drawable: Drawable, size: Vector2, position: Vector2): Unit = {
    ...
    if((lower.abs+upper.abs).x < 1 && (lower.abs+upper.abs).y < 1) {
      drawable.drawRectanglePartialProto((size-lower.abs-upper.abs)*:gridUnit, relativePosition+(position-lower)*:gridUnit, lower.abs, upper.abs)
    }
  }

  def drawGrid(drawables: Array[Array[Drawable]], offset: Vector2 = Vector2(0)): Unit = {
    cells = Vector2(drawables.length-2, drawables(0).length-2)
    for(x <- drawables.indices; y <- drawables(x).indices) {
      drawOnGrid(drawables(x)(y), Vector2(1), Vector2(x-1,y-1)+offset)
    }
  }
